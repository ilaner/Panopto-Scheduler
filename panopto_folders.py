...
class PanoptoFolders:
    ...
    def get_children(self, folder_id):
        ...
        page_number = 0
        while True:
            url = 'https://{0}/Panopto/api/v1/folders/{1}/children?pageNumber={2}&sortField=Name&sortOrder=Asc'.format(
                self.server, folder_id, page_number)
    ...
    def get_sessions(self, folder_id):
        ...
        page_number = 0
        while True:
            url = 'https://{0}/Panopto/api/v1/folders/{1}/sessions?pageNumber={2}&sortField=CreatedDate&sortOrder=Desc'.format(
                self.server, folder_id, page_number)
            resp = self.requests_session.get(url=url)
            if self.__inspect_response_is_retry_needed(resp):
                continue
